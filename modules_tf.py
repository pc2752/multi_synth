...
def full_network(prev, inputs, is_train):
    ...
    prev = tf.layers.batch_normalization(tf.layers.dense(prev, config.filters
        , name = "Prev_in", kernel_initializer=tf.random_normal_initializer(stddev=0.02)), training = is_train)

    output = encoder_decoder_archi_gan(inputs, is_train)

    output = tf.concat([tf.squeeze(output), prev], axis = -1)


    output = tf.tanh(tf.layers.batch_normalization(tf.layers.dense(output, 1, name = "Fu_F", kernel_initializer=tf.random_normal_initializer(stddev=0.02)), training = is_train, name = "bn_fu_out"))

    return output

def discriminator(out, prev, inputs, is_train):

    inputs = tf.concat([out, prev, inputs], axis = -1)
